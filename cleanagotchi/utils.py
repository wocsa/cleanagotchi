...
def load_config(args):
    ...
    ref_defaults_file = os.path.join(os.path.dirname(cleanagotchi.__file__), 'defaults.yml')
    ...
    # check for an entire cleanagotchi folder on /boot/
    if os.path.isdir('/boot/cleanagotchi'):
        print("installing /boot/cleanagotchi to /etc/cleanagotchi ...")
        shutil.rmtree('/etc/cleanagotchi', ignore_errors=True)
        shutil.move('/boot/cleanagotchi', '/etc/')
...
def extract_from_pcap(path, fields):
    ...
    for field in fields:
        ...
        if field == WifiInfo.BSSID:
            ...
            from cleanagotchi.mesh.wifi import freq_to_channel
