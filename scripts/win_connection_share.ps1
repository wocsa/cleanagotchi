<#
    .SYNOPSIS
        A script that setups Internet Connection Sharing for cleanagotchi.
    ...
    .PARAMETER SetcleanagotchiSubnet
        Change the Internet Connection Sharing subnet to the cleanagotchi subnet. The USB Gadget Interface IP will default to 10.0.0.1.
    ...
    .EXAMPLE
        # Change the regkeys of Internet Connection Sharing to the cleanagotchi Subnet
        PS C:\> .\win_connection_share -SetcleanagotchiSubnet

    .EXAMPLE
        # Change the regkeys of Internet Connection Sharing to the cleanagotchi Subnet with a custom ScopeAddress (The IP Address of the USB Gadget Interface.)
        PS C:\> .\win_connection_share -SetcleanagotchiSubnet -ScopeAddress 10.0.0.10
#>
...
Function Test-cleanagotchiSubnet {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        PS> Test-cleanagotchiSubnet
        [!] By default Internet Connection Sharing uses a 192.168.137.x subnet. Run Set-cleanagotchiSubnet to ensure you and your little friend are on the same subnet.
    #>
    [Cmdletbinding()]
    ...
    If ($RegKeys.ScopeAddress -notmatch '10.0.0.') {
        Write-Error "By default Internet Connection Sharing uses a 192.168.137.x subnet. Run Set-cleanagotchiSubnet to ensure you and your little friend are on the same subnet." -ErrorAction Stop
    }
    If ($RegKeys.ScopeAddressBackup -notmatch '10.0.0.') {
        Write-Error "By default Internet Connection Sharing uses a 192.168.137.x subnet. Run Set-cleanagotchiSubnet to ensure you and your little friend are on the same subnet." -ErrorAction Stop
    } 
}
Function Set-cleanagotchiSubnet {
    <#
    .SYNOPSIS
...
# Main Function
Function Setup-cleanagotchiNetwork {
    <#
    .SYNOPSIS
    ...
    .PARAMETER SetcleanagotchiSubnet
        Change the Internet Connection Sharing subnet to the cleanagotchi. Defaults to 10.0.0.1.
    
    .PARAMETER ScopeAddress
        Custom ScopeAddress (the ICS ip address)
    
    .EXAMPLE
        PS> Setup-cleanagotchiNetwork -EnableInternetConnectionSharing
    ...
    Begin {
        Try {
            ...
            Try {
                Test-cleanagotchiSubnet @ErrorSplat
            } Catch {
                If ($SetcleanagotchiSubnet) {
                    Write-Debug "Setting subnet"
                    Set-cleanagotchiSubnet -ScopeAddress $ScopeAddress @ErrorSplat
                } Else {
                    Write-Error "By default Internet Connection Sharing uses a 192.168.137.x subnet. Run this script with the -SetcleanagotchiSubnet to setup the network." -ErrorAction Stop
                }
            }
        } Catch {
            $PSCmdlet.ThrowTerminatingError($PSItem)
...
# Dynamically create params for Setup-cleanagotchiNetwork function based of param input of script.
Setup-cleanagotchiNetwork @psBoundParameters
